...
if(isset($_POST['submit'])) {
    ...
    // validate form data
    if(!$title) {
        $_SESSION['add-post'] = "პოსტის სათაური საჭიროა!";
    } elseif (!$category_id) {
        $_SESSION['add-post'] = "მიუთითეთ პოსტის კატეგორია!";
    } elseif(!$body) {
        $_SESSION['add-post'] = "პოსტის დეტალური აღწერა საჭიროა!";
    } elseif(!$thumbnail['name']) {
        $_SESSION['add-post'] = "მიუთითეთ პოსტის ფოტო!";
    } else {
        // work on thumbnail
        // rename the image
        ...
        if(in_array($extension, $allowed_files)) {
            ...
            if($thumbnail['size'] < 2_000_000) {
                ...
                $_SESSION['add-post'] = "ფოტოს ზომა არუნდა იყოს 2MB+";
            }
        } else {
            $_SESSION['add-post'] = "დასაშვებია მხოლოდ png, jpg ან jpeg";
        }
    }
    ...
    if(isset($_SESSION['add-post'])) {
        ...
        if(!mysqli_errno($connection)) {
            $_SESSION['add-post-success'] = "ახალი პოსტი დაემატა!";
